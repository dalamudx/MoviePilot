...
class SubscribeChain(ChainBase):
        ...
        **kwargs,
        ...
        if mediainfo.type == MediaType.TV:
            ...
            if not kwargs.get("lack_episode"):
                kwargs.update({"lack_episode": kwargs.get("total_episode")})
        else:
            # 避免season为0的问题
            season = None
    ...
    def __parse_history_episodes(episode_str: Optional[str]) -> List[int]:
        ...
        try:
            ...
            # 修复：使用 r"\d{1,4}" 匹配数字，而非错误的 r"\\d{1,4}"（字面量反斜杠+d）
    ...
    def __get_transferred_from_history(self, subscribe: Subscribe) -> List[int]:
        ...
        histories = []
        if subscribe.tmdbid:
            # 构建季查询参数（格式与数据库一致，如 "S01"），减少无效数据量
            season_str = f"S{subscribe.season:02d}" if subscribe.season else None
            histories = TransferHistoryOper().get_by(
                tmdbid=subscribe.tmdbid, mtype=subscribe.type, season=season_str
            )
        elif subscribe.name and subscribe.year:
            season_str = f"S{subscribe.season:02d}" if subscribe.season else None
            histories = TransferHistoryOper().get_by(
                title=subscribe.name, year=subscribe.year, mtype=subscribe.type, season=season_str
            )
        ...
        for history in histories:
            ...
            # 没有集数信息时，尝试按整季兜底（需已知总集数）
            # 防御：只有当 history.episodes 确实为空/None 时才允许整季兜底，
            # 避免 episodes 字段有值但解析失败时错误地将整季标记为已完成
            if not history.episodes and subscribe.total_episode:
                start = subscribe.start_episode or 1
                end = subscribe.total_episode
                if end >= start:
