...
async def update_subscribe(
    *,
    ...
    old_total_episode = subscribe.total_episode
    # 避免更新缺失集数
    ...
    # 是否手动修改过总集数
    total_episode_changed = subscribe_in.total_episode != old_total_episode
    if total_episode_changed:
        subscribe_dict["manual_total_episode"] = 1
        if subscribe_in.total_episode is not None:
            subscribe_dict["lack_episode"] = (
                SubscribeProgressOper.calc_target_episode_count(
                    subscribe_in.start_episode or subscribe.start_episode,
    ...
    progress = await SubscribeProgress.async_get_by_subscribe_id(db, subscribe_in.id)
    if progress and total_episode_changed:
        await progress.async_update(
            db,
            {
                "total_episode": subscribe_in.total_episode,
            },
        )
    # 重新获取更新后的订阅数据
        ...
        await subscribe.async_update(
            db,
            {
                "lack_episode": SubscribeProgressOper.calc_target_episode_count(
                    ...
                    subscribe.total_episode,
                    subscribe.type,
                ),
                "state": "R",
            },
        )
        progress = await SubscribeProgress.async_get_by_subscribe_id(db, subid)
        if progress:
            await progress.async_update(
                db,
                {
