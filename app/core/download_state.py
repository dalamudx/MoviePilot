...
# 修复：统一已完成状态列表常量，避免各方法间不一致
# 含义：所有 qBittorrent 下载完成后可能处于的状态（*UP 系列 + 通用完成状态）
COMPLETED_DOWNLOAD_STATES = frozenset([
    'completed',   # 通用完成状态
    'seeding',     # 通用做种状态
    'uploading',   # 正在做种并传输数据
    'stalledup',   # 正在做种但无连接
...
class DownloadStateManager(metaclass=SingletonClass):
    ...
    def is_torrent_fully_downloaded(self, hash_str: str, torrent_info: dict = None) -> bool:
        ...
        # 检查种子状态（使用统一常量）
        state = (torrent_info.get('state') or '').lower()
        if state not in COMPLETED_DOWNLOAD_STATES:
            return False
    ...
    def get_error_recovery_suggestions(self, hash_str: str, torrent_info: dict = None) -> List[str]:
        ...
        elif state not in COMPLETED_DOWNLOAD_STATES:
    ...
    def reset_transfer_state(self, hash_str: str) -> bool:
        """重置已整理完成状态（仅删除transferred缓存）"""
        if not hash_str:
            return False
        ...
        if state_data.get("state") != DownloadLifecycle.TRANSFERRED.value:
            return False
        ...
        logger.info(f"重置已整理状态: {state_data.get('title')} - {hash_str}")
        return True

    def clear_all_transferred_states(self) -> int:
        """清理所有已整理完成的状态缓存"""
        cleared = 0
        hashes = self._get_all_transferred_hashes()
        if not hashes:
            return 0
        ...
        if cleared:
            logger.info(f"清理已整理状态缓存 {cleared} 条")
        return cleared
    ...
    def is_transfer_completed(self, hash_str: str) -> bool:
        """检查是否已整理完成"""
        # 修复：hash 为空不代表已完成，应返回 False 避免跳过整理
        if not hash_str:
            return False
    ...
    def get_transferred_episodes(self, tmdbid: int = None, doubanid: str = None,
        ...
        # 修复：将 int 季号统一转为 "S01" 格式，与 mark_downloading 写入的 meta.season 格式对齐
        # mark_downloading 中存储的是 meta.season（MetaBase.season property），格式为 "S01"
        ...
        for hash_str in self._get_all_transferred_hashes():
            ...
            # 电视剧：按季过滤
            if media_type == "tv":
                if season_str and state_data.get("season") != season_str:
                    continue
                episodes = state_data.get("episodes", [])
