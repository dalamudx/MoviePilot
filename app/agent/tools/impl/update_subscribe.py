        ...
        **kwargs,
        ...
        try:
            ...
            async with AsyncSessionFactory() as db:
                ...
                total_episode_changed = False
                if total_episode is not None and total_episode != old_total_episode:
                    total_episode_changed = True
                    subscribe_dict["manual_total_episode"] = 1
                    subscribe_dict["lack_episode"] = (
                        SubscribeProgressOper.calc_target_episode_count(
                            start_episode
                            if start_episode is not None
                            else subscribe.start_episode,
                            total_episode,
                            subscribe.type,
                        )
                    )
                ...
                if total_episode_changed:
                    progress = await SubscribeProgress.async_get_by_subscribe_id(
                        db, subscribe_id
                    )
                    if progress:
